...
class MCTree_Handler:
    ...
    def __init__(self,n_parallel=8,device=torch.device('cuda' if torch.cuda.is_available() else "cpu"),max_game_ply=160):

        self.lookup_dict            = {}
        self.active_boards          = [MCTree(max_game_ply=max_game_ply,lookup_dict=self.lookup_dict) for _ in range(n_parallel)]
    ...
    #Performs an algorithm iteration until all nodes are waiting for a GPU eval
    def perform_iter_to_eval(self):

        #Wait until everyone needs a gpu
        while False in [tree.gpu_blocking for tree in self.active_boards]:

        pass
class MCTree:


    def __init__(self,max_game_ply=160,lookup_dict={}):
        ...
        self.active_boards          = [chess.Board() for _ in range(n_parallel)]
    ...
    def perform_nongpu_iter(self):
        ...
        #Node key
        curnode.key                     = " ".join(self.board.fen().split(" "))

        #Curnode now needs to be expanded
        if curnode.key in self.lookup_dict:
            self.perform_nongpu_expansion(curnode)

    def perform_nongpu_expansion(self,node:Node):
